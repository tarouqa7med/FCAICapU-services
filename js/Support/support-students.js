/*
  Support Students page logic

  - Fetch and render statistics (analysis numbers) from backend PHP
  - Update numbers in .analysis-box and tiles where present
  - Set dataset metadata on each back button so payment flow can identify category/index

  Auth gating + payment popup is handled by:
  - js/Support/backProjectAuth.js
  - js/pay_method.js
*/
...
(function () {
  ...
  const PAGE_CATEGORY = 'students';

  function getPhpUrl() {
    const path = window.location.pathname.toLowerCase();
    if (path.includes('/html/')) return '../php/Support/support-students.php';
    return './php/Support/support-students.php';
  }
  ...
  function bindProjectTileMeta() {
    const section = document.querySelector('section.support[data-category="students"]') || document;
